...
internal fun <T : Annotation> Class<*>.findAnnotation(ac: Class<T>): T? { // unfortunately, the class discovery algorithm of ServletContainerInitializer+@HandlesTypes is quite weak.
...
private fun String.shouldPrependHyphen(i: Int): Boolean = if (i == 0) { // Never put a hyphen at the beginning
  false
}
else if (!Character.isUpperCase(this[i - 1])) { // Append if previous char wasn't upper case
  true
}
