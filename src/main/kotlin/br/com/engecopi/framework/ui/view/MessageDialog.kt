...
object MessageDialog {
  ...
  fun warning(caption: String = "Aviso", message: String) {
    ...
    else MessageBox.createWarning().withCaption(caption).withHtmlMessage(message)
      .withCloseButton(ButtonOption.caption("Fechar")).asModal(true).open()
  }
  ...
  fun error(caption: String = "Erro", message: String) {
    ...
    else MessageBox.createError().withCaption(caption).withHtmlMessage(message)
      .withCloseButton(ButtonOption.caption("Fechar")).asModal(true).open()
  }
  ...
  fun image(title: String, image: ByteArray, printRunnable: Runnable) {
    val resource = image.makeResource()
    val embedded = Embedded()
    embedded.type = Embedded.TYPE_BROWSER
    embedded.source = resource
    MessageBox.create().withCaption(title).withMessage(embedded).withHeight("100%").withWidth("100%")
      .withCustomButton(printRunnable, ButtonOption.caption("Imprimir"), ButtonOption.icon(VaadinIcons.PRINT))
      .withCloseButton(ButtonOption.caption("Fecha")).open()
  }
}