...
class LabelProdutoViewModel(view: ILabelProdutoView) : ViewModel<ILabelProdutoView>(view) {
  ...
  fun addFaixaCodigoGrade(codigo: String?, grade: String?) = execUnit {
    ...
    if (localizacoes.isNotEmpty() && !view.listaProduto.contains(produto)) view.listaProduto += produto
  }
  ...
  fun impressaoProduto(): String? {
    ...
    return view.listaProduto.joinToString(separator = "\n") { prd ->
      ...
      barcodeGtin.joinToString(separator = "\n") { bar ->
        val print =
          template.replace("[Codigo]", prd.codigo.trim()).replace("[Grade]", prd.grade)
