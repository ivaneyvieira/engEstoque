...
class ViewCodBarConferencia {
  ...
  companion object Find: ViewCodBarConferenciaFinder() {
    private fun findNotaCodbarCliente(key: String): ViewCodBarConferencia? {
      return ViewCodBarCliente.where().codbarLimpo.eq(key).findList().firstOrNull()?.run {
        where().storeno.eq(storeno).numero.eq(numero).sequencia.eq(sequencia)
          .abreviacao.eq(RegistryUserInfo.abreviacaoDefault).findList().firstOrNull()
        }
    }

    private fun findNotaCodBarConferencia(key: String): ViewCodBarConferencia? {
      return where().codbar.eq(key).findList().firstOrNull()
    }
    ...
    fun findKeyItemNota(key: String): List<ItemNota> {
      return where().codbar.eq(key).findList().mapNotNull {ItemNota.byId(it.id)}
    }
  }
}
