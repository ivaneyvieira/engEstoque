...
@Entity
@View(name = "v_pedido_ressuprimento", dependentTables = ["notas", "itens_nota"]) class ViewPedidoRessuprimento :
        BaseModel() {
  ...
  companion object Find : ViewPedidoRessuprimentoFinder() {
    fun findSaida(loja: Loja, numero: String?, abreviacao: String?): ViewPedidoRessuprimento? {
      ...
      return QViewPedidoRessuprimento().numero.eq(numero).loja.equalTo(loja).abreviacao.eq(abreviacao)
              .findList()
              .firstOrNull()
    }

    fun findExpedicao(nota: Nota): ViewPedidoRessuprimento? {
      return QViewPedidoRessuprimento().numero.eq(nota.numero).loja.equalTo(nota.loja).findList().firstOrNull()
    }
  }
