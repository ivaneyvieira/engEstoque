...
class TransferenciaAutomatica(
  ...
  companion object {
    fun notaFatura(lojaTransferencia: Int?, numeroSerieTransferencia: String?): List<NotaBaixaFatura> {
      ...
      """.trimMargin()
      return DB.findDto(TransferenciaAutomatica::class.java, sql).setParameter("lojaTransferencia", lojaTransferencia)
        .setParameter("numeroSerieTransferencia", numeroSerieTransferencia).findList().map { nf ->
          NotaBaixaFatura(
            nf.storenoFat, nf.nffat, nf.data.localDate()
          )
        }
    }
    ...
    fun notaBaixa(lojaFatura: Int?, numeroSerieFatura: String?): List<NotaBaixaFatura> {
      ...
      """.trimMargin()
      return DB.findDto(TransferenciaAutomatica::class.java, sql).setParameter("lojaFatura", lojaFatura)
        .setParameter("numeroSerieFatura", numeroSerieFatura).findList().map { nf ->
          NotaBaixaFatura(
            nf.storenoTransf, nf.nftransf, nf.data.localDate()
          )
        }
    }
  }
