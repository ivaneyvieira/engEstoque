...
class EntregaFutura(
  ...
  companion object {
    fun notaBaixa(storeno: Int?, numeroVenda: String?): List<NotaBaixaFatura> {
      ...
      """.trimMargin()
      val list =
        DB.findDto(EntregaFutura::class.java, sql).setParameter("storeno", storeno)
          .setParameter("numeroVenda", numeroVenda).findList().map { nf ->
            NotaBaixaFatura(
              nf.storenoEntrega, nf.numeroEntrega, nf.dataEntrega.localDate()
            )
          }
      return list;
    }
    ...
    fun notaFatura(storeno: Int?, numero: String?): List<NotaBaixaFatura> {
      ...
      """.trimMargin()
      return DB.findDto(EntregaFutura::class.java, sql).setParameter("storeno", storeno).setParameter("numero", numero)
        .findList().map { nf ->
          NotaBaixaFatura(
            nf.storenoVenda, nf.numeroVenda, nf.dataVenda.localDate()
          )
        }
    }
  }
