...
class ViewProdutoLoc(
  ...
  companion object Find : ViewProdutoLocFinder() {
    ...
    fun localizacoesProduto(produto: Produto?): List<String> {
      produto ?: return emptyList()
      return QViewProdutoLoc().produto.id.eq(produto.id).findList().asSequence().mapNotNull { it.localizacao }
        .distinct().map { localizacao ->
    ...
    fun findByCodigoGrade(prdno: String?, grade: String?): List<ViewProdutoLoc> {
      ...
      return QViewProdutoLoc().codigo.eq(prdno.padStart(16, ' ')).grade.eq(grade).findList()
    }
  }
}
