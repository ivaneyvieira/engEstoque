...
object Repositories {
  ...
  private fun atualizaRepositorio() {
    ...
    val list = QViewProdutoLoc().fetch("produto").fetch("loja").findList().toList()
...
"""
  ...
  fun findByProduto(produto: Produto?): List<ViewProdutoLoc> {
    ...
    return viewProdutosLocProdutoKey[ProdutoKey(
      produtoId = produto.id, storeno = numero, abreviacao = abreviacaoDefault
    )].orEmpty()
  }
  ...
  fun findByLojaAbreviacao(
    storeno: Int = lojaDeposito.numero, abreviacao: String = abreviacaoDefault
  ): List<ViewProdutoLoc> {
