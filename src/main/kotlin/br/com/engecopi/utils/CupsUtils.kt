...
fun print(printerName: String, text: String, resultMsg: (String) -> Unit) {
  val cupsClient = CupsClient()
  cupsClient.printers.firstOrNull {it.name == printerName}
    ?.let {printer ->
  ...
  resultMsg("Impressora $printerName não foi encontrada no sistema")
}
