CREATE OR REPLACE VIEW v_pedido_ressuprimento AS
SELECT max(I.id) AS id, I.nota_id AS nota_id, N.numero AS numero, N.tipo_mov AS tipo_mov,
       ...
       substring_index(I.localizacao, '.', 1) AS abreviacao, I.codigo_barra_conferencia
FROM (notas N
  JOIN itens_nota I
         ON ((I.nota_id = N.id)))
WHERE ((I.status = 'INCLUIDA') AND (N.tipo_nota = 'PEDIDO_R'))
